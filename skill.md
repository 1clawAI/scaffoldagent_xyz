...
description: >-
  ...
  swarm, 1Claw/Shroud, just commands, and security. Upstream source lives in
  ~/scaffold-agent (or https://github.com/1clawAI/scaffold-agent).
---

# scaffold-agent — AI / agent instructions

This file documents the **`scaffold-agent`** CLI published on npm ([scaffold-agent](https://www.npmjs.com/package/scaffold-agent)). It is the **generator** that writes new project directories; a repo containing only this `skill.md` is **not** the CLI source tree unless you also have the CLI cloned.

**Upstream reference:** clone or read **`~/scaffold-agent`** — authoritative files there are **`AGENTS.md`**, **`README.md`**, and **`.cursor/skills/scaffold-agent/`** (mirrors much of this content).
...
- Generated apps follow patterns similar to [Scaffold-ETH 2](https://github.com/scaffold-eth/scaffold-eth-2) (wagmi, ABIs, `/debug` on Next, etc.) when those pieces are included in the template.
...
- **[just](https://just.systems)** installed on the host (not an npm dependency of the CLI). Generated repos use a root **`justfile`** for `just chain`, `just deploy`, `just start`, etc. Without `just`, run the equivalent **`npm run`** scripts from the generated **`README.md`**.
...
**From a local clone** of `~/scaffold-agent`:
...
After any change under **`src/`** in the generator repo, run **`npm run build`** before testing **`dist/cli.js`**.
...
- Defaults under `-y` match **`scaffold-agent --help`** (e.g. Foundry + Next.js + 1Claw Shroud + token billing unless overridden).

**Shroud + `-y` validation (summary):**
...
- Implementation reference in generator repo: **`src/agent-project-config.ts`**.
...
- **`scripts/`** — `fund-deployer.mjs`, `swarm-agents.mjs`, `with-secrets.mjs`, deploy scripts, etc.
- **`packages/foundry`** or **`packages/hardhat`** (if chain chosen)
- **`packages/nextjs`** or **`packages/vite`** or **`packages/python`** — chat UI, **`/api/chat`**, identity/ENS/balances/swarm routes as applicable
...
| `just reset` | Re-bootstrap 1Claw vault + agent (when 1Claw; **backup `.env` first**) |
...
- **`SHROUD_LLM_PROVIDER`**: `openai`, `anthropic`, `google` / `gemini`, `mistral`, `cohere`, `openrouter`, etc.
- **`SHROUD_BILLING_MODE`**: `token_billing` (1Claw billing) vs `provider_api_key` (BYOK in vault or **`SHROUD_PROVIDER_API_KEY`**).

Generated README in **`~/scaffold-agent`** lists default Shroud models per upstream and direct Gemini/OpenAI/Anthropic defaults for non-Shroud paths.
...
## Maintaining the generator (CLI repo only)

| Area | Path (in `~/scaffold-agent`) |
|------|------------------------------|
| CLI entry | `src/cli.ts` |
| Args / defaults | `src/cli-argv.ts` |
| Wizard / `-y` | `src/cli-wizard.ts` |
| `agent.json` / dump / swarm plan | `src/agent-project-config.ts` |
| File generation + large templates | `src/actions/scaffold.ts` |
| Reusable snippets | `src/scaffold-templates/*.ts` |

Pinned SDK versions for generated apps: **`AMPERSEND_SDK_VERSION`**, **`ONECLAW_SDK_VERSION`** in **`src/actions/scaffold.ts`**.
...
*Synthesized from `~/scaffold-agent` (SKILL.md, reference.md, README.md, AGENTS.md). If behavior diverges, trust **`scaffold-agent --help`** and the upstream repo.*
